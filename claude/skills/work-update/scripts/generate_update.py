...
ARTIFACT_DIRS = ("docs/superpowers", ".ai", ".omo", ".gsd", ".codex")
...
def default_ref(repo):
    ...
    return "origin/main" if has_origin_main else "HEAD"
...
def is_main_branch(branch):
    return branch in {"main", "staging"} or not branch
...
def cherry_lines(repo):
    out = run(["git", "cherry", "-v", "origin/main", "HEAD"], cwd=repo, check=False)
    plus = []
    minus = []
    for line in out.splitlines():
        if line.startswith("+ "):
...
def progress_from_text(text):
    ...
    percent = percents[-1] if percents else checkbox_percent
...
def in_progress_items(
    ...
    for entry in worktrees(repo_root):
        ...
        if not repo or not repo.exists() or entry.get("prunable") or branch is None or is_main_branch(branch):
            continue
        ...
        plus, _minus = cherry_lines(repo)
        if not plus and not dirty:
            continue
        ...
        if not commit_active and not dirty_active:
            continue
...
def write_context_markdown(
    ...
    commit_groups = commits_by_project(commits)
    progress_groups = progress_by_project(in_progress)
    dev_commits = dev_commits or []
    dev_by_project = commits_by_project(dev_commits)
    for project in PROJECT_ORDER:
        project_commits = commit_groups.get(project, [])
        project_progress = progress_groups.get(project, [])
        project_dev_commits = dev_by_project.get(project, [])
        lines.append(f"## {project}")
        lines.append("")
        ...
        if project_dev_commits:
            lines.append(f"### Dev Branch Activity ({len(project_dev_commits)})")
            lines.append("Commits on the project dev branch that are not yet on origin/main.")
            lines.append("")
            for commit in project_dev_commits:
                write_commit_context(lines, commit)
...
def main():
    ...
    parser.add_argument("--max-in-progress", type=int, default=2)
    parser.add_argument(
        "--in-progress",
        action="append",
        default=[],
    ...
    dev_commits = []
    for source in sources:
        ref = source.ref or default_ref(source.path)
        for commit in get_commits_in_bounds(source.path, ref, since, until):
            commit["source"] = source.name
            commit["project"] = source.project
            if source.name == "Gondoor Dev":
    ...
    if dev_commits:
        gondoor_landed_texts = landed_texts.get("Gondoor", [])
        dev_commits = [
            commit
            for commit in dev_commits
            if not (
                is_ancestor(commit_repo(commit), commit["sha"], "origin/main")
                or any(
                    normalized_sentence(cleaned_subject(commit["subject"])) in landed_text
                    for landed_text in gondoor_landed_texts
                )
            )
        ]
    ...
    if args.show_commits:
        for commit in commits + dev_commits:
            print(f"{commit['source']}: {commit['sha']} {commit['date']} {commit['subject']}")
        print(f"Included commits: {len(commits) + len(dev_commits)}")
