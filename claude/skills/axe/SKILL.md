...
## Local notes — this machine (Xcode 26, iOS 27, XcodeBuildMCP removed)

Measured here, not upstream. Where these conflict with the defaults above, these win.

- **Always pass `--tap-style physical`; the default `automatic` silently no-ops.** Proven with
  fixed coordinates, warm session, Settings root, nothing else changed:
  - `axe tap -x 201 -y 319`                     -> `Tap at (201.0, 319.0) completed successfully`, screenshot byte-identical.
  - `axe tap -x 201 -y 319 --tap-style physical` -> screen changed (sheet opened for the row at that point).

  `completed successfully` only means the event was dispatched. Confirm every navigation with
  `describe-ui` or `cmp -s before.png after.png` before trusting the next step.
- **First call after boot times out.** `Error: Timed out creating the simulator remote automation
  session` on the first `describe-ui` following a fresh boot; retry once and it returns the full
  tree (reproduced 3x). `testmanagerd` is warming up — not a broken install.
- **`describe-ui` can lag the live screen.** Straight after boot, iOS shows a first-run
  "Optimizing Search and Siri" card that appears then disappears. The tree reported `General` at
  y=319.33 while the live screen had that card at y=319. Re-snapshot before resolving a selector,
  and cross-check with a screenshot when a tap appears to miss.
- **AX frames are already in HID point space — no offset maths.** Verified: the root application
  frame is `402x874`, equal to the screenshot point size (1206x2622 px at @3x), and a row's frame
  centre matched its pixel-measured centre. `describe-ui` coordinates can be passed straight to
  `tap -x -y`.
- **Never dump the whole tree into context.** `describe-ui` returns 300-400 KB of JSON (~75-100k
  tokens). Use `describe-ui --point X,Y` for a single element, or filter first:
  `jq -r '.. | objects | select(.AXLabel != null and .AXLabel != "") | .AXLabel' | sort -u`
  (about 34 labels, ~500 tokens) before deciding what to tap.
- **`simctl` cannot inject input.** `simctl io` supports only `enumerate`, `poll`, `recordVideo`,
  `screenshot`, `screenConfig power|geometry`. Use simctl for boot/install/launch/screenshot and
  AXe for every tap/type/key/swipe.
- **`xcodebuildmcp` is not installed** (removed 2026-09-18). Use `xcodebuild` for builds and
  `simctl install` / `simctl launch` for installing and launching; AXe covers all simulator UI work.
- **Simulator.app does not ship with Xcode 26.** Device Hub
  (`Xcode.app/Contents/Applications/DeviceHub.app`) replaces it, and AXe does not need it.
