...
function inProcessFetchFromClient(sdk: unknown): ((req: Request) => Promise<Response>) | null {
  try {
    const raw = sdk as { _client?: { getConfig?: () => { fetch?: unknown } } }
    const fn = raw?._client?.getConfig?.()?.fetch
    return typeof fn === "function" ? (fn as (req: Request) => Promise<Response>) : null
  } catch {
    return null
  }
}
  ...
  try {
    ...
    const relay = bun.serve({
      ...
      fetch(req: Request): Promise<Response> | Response {
        ...
        const target = new URL(url.pathname + url.search, OPENCODE_UNBOUND_SERVER_URL)
...
async function refreshOpenCodeModelLimits(client: Record<string, unknown>, directory: string): Promise<void> {
  ...
  try {
    ...
    const data = result && typeof result === "object" && "data" in result ? (result as Record<string, unknown>).data : result
    const list = data && typeof data === "object" && Array.isArray((data as Record<string, unknown>).providers)
      ? (data as Record<string, unknown>).providers as Array<Record<string, unknown>>
      : []
    for (const provider of list) {
      ...
      for (const [modelID, raw] of Object.entries(models as Record<string, unknown>)) {
        ...
        const context = limit && typeof limit === "object" ? (limit as Record<string, unknown>).context : undefined
...
function tokenTotal(tokens: unknown): number {
  ...
  const direct = ["input", "output", "reasoning"].reduce((sum, key) => sum + (typeof rec[key] === "number" ? rec[key] as number : 0), 0)
...
function openCodeContextRemainingFromDB(sessionID: string): number {
  ...
  try {
    ...
    const row = db.query("select model, tokens_input, tokens_output, tokens_reasoning from session where id = ? limit 1")
    ...
    const window = modelContextLimits.get(model.providerID + "\0" + model.modelID) ?? modelContextLimits.get(model.modelID) ?? 0
...
function openCodeContextRemainingFromMessageFiles(sessionID: string): number {
  let files: string[]
  const dir = pathJoin(resolveOpenCodeDataDir(), "storage", "message", sessionID)
  try {
  ...
  files.sort((a, b) => {
    try {
      return statSync(pathJoin(dir, b)).mtimeMs - statSync(pathJoin(dir, a)).mtimeMs
    } catch {
  ...
  for (const file of files) {
    ...
    const window = modelContextLimits.get(providerID + "\0" + modelID) ?? modelContextLimits.get(modelID) ?? 0
  ...
  if (typeof raw === "string" && raw.length > 0) {
    try {
      ...
      if (parsed && typeof parsed === "object") {
        ...
        return { providerID: stringProp(rec, "providerID"), modelID: stringProp(rec, "id", "modelID") }
      }
    } catch {}
  }
...
function withContextRemaining(envelope: Record<string, unknown>, sessionID: string | undefined): Record<string, unknown> {
...
function sendSessionClosed(eventName: string, sessionID: string | undefined, directory: string): void {
  void modelLimitsReady.finally(() => {
    void sendEnvelope(
      {
        type: "session.closed",
...
function sendIdleIfActive(eventName: string, sessionID: string | undefined, cwd: string): void {
  ...
  sendSessionUpdate(eventName, sessionID, cwd, undefined, "task_complete", firstValue(lastAssistantTitles, keys) || "OpenCode idle", prompt)
}
...
function rememberUserPrompt(sessionID: string | undefined, cwd: string, prompt: string): string {
...
function titleForPromptStart(sessionID: string | undefined, cwd: string): string {
...
function rememberAssistantText(sessionID: string | undefined, cwd: string, messageID: string, text: string, append: boolean): void {
...
function summarizeQuestion(props: Record<string, unknown>): string {
  const direct = stringProp(props, "question", "prompt", "message", "text", "title")
  if (direct) return direct
  const nested = props.question
  if (nested && typeof nested === "object") {
    const nestedText = stringProp(nested as Record<string, unknown>, "question", "prompt", "message", "text", "title")
...
function requestMoshiApproval(
  ...
  const promise = modelLimitsReady.then(() => sendEnvelope(
    withContextRemaining({
  ...
  event: async ({ event }) => {
    ...
    switch (event.type) {
      ...
      case "message.part.delta": {
        ...
        if (field === "text") rememberAssistantText(sessionIDFromProperties(props), cwd, messageID, delta, true)
        break
      }
      case "message.part.updated": {
        ...
        if (part && typeof part === "object") {
          ...
          if (rec.type === "text" && typeof rec.text === "string") rememberAssistantText(sessionIDFromProperties(props), cwd, messageID, rec.text, false)
        }
        break
      }
      case "session.status": {
        ...
        if (statusType === "busy") {
          ...
          sendSessionUpdate("session.status", sessionID, cwd, undefined, "", "OpenCode started", statusType)
        }
        break
      }
      case "session.idle": {
      ...
      case "permission.replied":
        ...
        break
      case "question.asked":
        sendTerminalInputRequired("question.asked", props, directory, "OpenCode needs input", "Input requested")
        break
      case "question.replied":
      case "question.rejected":
        sendSessionUpdate(event.type, sessionIDFromProperties(props), directory)
        break
      case "session.error":
        sendTerminalInputRequired("session.error", props, directory, "OpenCode blocked", "OpenCode reported an error")
        break
      default:
        break
    }
