...
```bash
node "$HOME/.gemini/get-shit-done/bin/gsd-tools.cjs" validate health $REPAIR_FLAG
```
...
```bash
node "$HOME/.gemini/get-shit-done/bin/gsd-tools.cjs" validate health
```
...
if [ -d "$TASKS_DIR" ]; then
  ...
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in $HOME/.gemini/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf $HOME/.gemini/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
