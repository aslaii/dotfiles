...
- **Global scope**: `$HOME/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `$HOME/.gemini/notes/`)
...
2. Glob `$HOME/.gemini/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `$HOME/.gemini/notes/` — works in any directory
