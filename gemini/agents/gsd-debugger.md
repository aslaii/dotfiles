...
Check code says:  hooksDir = path.join(configDir, 'hooks')
                  configDir = ~/.claude
                  → checks $HOME/.gemini/hooks/

Installer says:   hooksDest = path.join(targetDir, 'hooks')
                  targetDir = $HOME/.gemini/get-shit-done
                  → writes to $HOME/.gemini/get-shit-done/hooks/
...
```bash
INIT=$(node "$HOME/.gemini/get-shit-done/bin/gsd-tools.cjs" state load)
...
```bash
node "$HOME/.gemini/get-shit-done/bin/gsd-tools.cjs" commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
...
```bash
node "$HOME/.gemini/get-shit-done/bin/gsd-tools.cjs" commit "docs: update debug knowledge base with {slug}" --files .planning/debug/knowledge-base.md
```
