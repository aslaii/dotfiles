...
**See @$HOME/.gemini/get-shit-done/references/checkpoints.md**
...
```bash
node $HOME/.gemini/get-shit-done/bin/gsd-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {concise task description}" --files file1 file2 ...
```
...
**Use template:** @$HOME/.gemini/get-shit-done/templates/summary.md
...
node "$HOME/.gemini/get-shit-done/bin/gsd-tools.cjs" roadmap update-plan-progress "$PHASE_NUMBER"
...
```bash
node "$HOME/.gemini/get-shit-done/bin/gsd-tools.cjs" state add-blocker "Blocker description"
```
</state_updates>
