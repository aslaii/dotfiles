...
8. Create PR:
   ...
   - **Never reference internal orchestration tooling** (`.omo/`, plan files, "boulder state", "AdversarialVerify", ledger paths, or any other agent-scratch-space vocabulary) in the title or body. Other engineers on the repo have zero context on that tooling. Describe the change, the reason, and how it was verified (tests, QA, screenshots) in plain engineering language, as if a human wrote it by hand. State the rigor (TDD, independent review, browser QA), never the tool that produced it.
   - Before opening, re-read the drafted body once specifically to grep it for `.omo`, "plan:", or similar internal paths and strip any hit.
