...
tmux send-keys -t "$SESSION_NAME":Servers.2 "cd ~/work/wsg-api && php artisan serve" C-m
tmux split-window -v -t "$SESSION_NAME":Servers.2

tmux send-keys -t "$SESSION_NAME":Servers.3 "cd ~/work/wsg-portal && PORT=3000 BROWSER=none HOST=wsg.local bun start:dev" C-m
...
tmux send-keys -t "$SESSION_NAME":API "cd ~/work/wsg-api && clear" C-m

# Create the Portal window
tmux new-window -t "$SESSION_NAME" -n Portal
tmux send-keys -t "$SESSION_NAME":Portal "cd ~/work/wsg-portal && clear" C-m
