...
PROJECT_ROOT="${1:-$HOME/work/JLabs/coden}"
...
tmux send-keys -t "$PANE_API" "cd \"$PROJECT_ROOT/coden-api/\" && php artisan serve" C-m
...
tmux send-keys -t "$PANE_WEB" "cd \"$PROJECT_ROOT/coden-portal/\" && pnpm dev" C-m
tmux select-pane -t "$PANE_WEB" -T "Web Server"

PANE_WEB=$(tmux split-window -v -t "$PANE_WEB" -P -F "#{pane_id}")
tmux send-keys -t "$PANE_WEB" "cd \"$PROJECT_ROOT/coden-cms-api/\" && pnpm start" C-m
tmux select-pane -t "$PANE_WEB" -T "CMS Server"
